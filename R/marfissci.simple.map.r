marfissci.simple.map<-function(rds,
  ...
  if (!exists("coast.aea") ||
    ...
    coast.aea =  aegis::coastline.db( DS="gshhg coastline highres",
                          crs=proj.metric,
  ...
  #bathy data
   ...
   p = aegis_parameters( DS="bathymetry" )
...
#    plot.title.clean=paste0(sub('_$', '', plot.title.clean),"_")
   ...
   png(filename=paste0(project.datadirectory("aegis", "data", ,out.folder, "figures" ), the.filename),
